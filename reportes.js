...
async function fetchCreditosHistorialPagos() {
  ...
  return (cuotas||[]).map(cu => {
    ...
    return {
      ...
      // El saldo real de ESTA cuota es lo que falta de pagar de ella
      // misma -- nunca el campo cu.saldo, que en realidad es el saldo
      // del cronograma completo (siempre da 0 en la última cuota,
      // sin importar si se pagó o no).
      saldo:          Math.max(0, Number(cu.monto_total||0) - Number(cu.monto_pagado||0)),
      estado:         cu.estado,
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  const intervalo = setInterval(() => {
    intentos++;
    if (R.empresaConfig || intentos >= 30) {
      clearInterval(intervalo);
      const btn = document.getElementById('btn-moneda-vis-texto');
      if (btn) btn.textContent = monedaParaMostrar(R.empresaConfig?.moneda);
    }
  }, 100);
});
