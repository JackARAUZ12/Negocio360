/* =====================================================
   ...
   (definido en caja.js). Caja sigue siendo la única
   fuente oficial de capital disponible.
...
let STATE = {
  ...
  gastosPerPage: 15,
  gastosTotal:   0,
  gastosFiltro:  'todos',     // todos | inmediatos | programados | pendientes | pagados | vencidos
  gastosSearch:  '',
  gastosCategoria: '',

  // Programados (para selects y vencimientos)
  gastosProgramados: [],

  // KPIs
  kpis: {
    hoy: 0, mes: 0, anio: 0, pendientes: 0, recurrentesActivos: 0, salariosPendientes: 0,
  },

  // UI
  activeSection: 'gastos', // gastos | programados
};
...
// Calcula la siguiente fecha de vencimiento a partir de una fecha base y frecuencia
...
/* =====================================================
   EMPRESA CONFIG / PERFIL (idéntico patrón a caja.js)
===================================================== */
...
/* =====================================================
   ADMIN ACCESS (idéntico patrón a caja.js / dashboard)
===================================================== */
...
/* =====================================================
   MÉTODOS DE PAGO (solo lectura — se gestionan en Caja)
===================================================== */
...
function populateMetodoSelects() {
  const selects = [document.getElementById('gasto-metodo'), document.getElementById('pago-metodo')];
  selects.forEach(sel => {
    if (!sel) return;
...
async function loadKpis() {
  try {
    ...
    const pendientesList = pendRes.data || [];
    // "Pendientes" = programados cuya próxima fecha ya venció (no se han pagado aún)
    const hoyDate = todayISO();
    STATE.kpis.pendientes = sum(pendientesList.filter(p => p.fecha_proxima <= hoyDate));

    STATE.kpis.recurrentesActivos = progActRes.count || 0;

    // Salarios pendientes: programados activos categoría Salarios con vencimiento <= hoy
...
async function loadGastos() {
  try {
    ...
    // 'todos' no agrega filtro de estado/tipo (incluye cancelados, atenuados visualmente)

    if (STATE.gastosCategoria) {
      query = query.eq('categoria', STATE.gastosCategoria);
    }
    if (STATE.gastosSearch.trim()) {
      query = query.ilike('concepto', `%${STATE.gastosSearch.trim()}%`);
    }
...
function renderGastosTable() {
      ...
      <tr>
        <td colspan="8" class="empty-cell">
          <div class="empty-state-mini">
            <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" style="opacity:0.3"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 0 0 0 7h5a3.5 3.5 0 0 1 0 7H6"/></svg>
  ...
  tbody.innerHTML = STATE.gastos.map(g => {
    ...
    // próximo vencimiento si está ligado a un programado
    const prog = g.gasto_programado_id ? STATE.gastosProgramados.find(p => p.id === g.gasto_programado_id) : null;
...
function renderPaginacion() {
  ...
  if (info) {
    ...
    info.textContent = STATE.gastosTotal > 0 ? `Mostrando ${from}–${to} de ${STATE.gastosTotal}` : 'Sin resultados';
  }
...
function paginaAnterior() {
  if (STATE.gastosPage > 1) { STATE.gastosPage--; loadGastos(); }
}
...
/* =====================================================
   GASTOS PROGRAMADOS — TABLA Y VENCIMIENTOS
===================================================== */
...
function renderGastosProgramados() {
  ...
  tbody.innerHTML = STATE.gastosProgramados.map(p => {
    ...
    <tr class="mov-row ${!p.activo ? 'mov-anulado' : ''}">
      ...
      <td>
        <span class="status-badge ${p.activo ? 'badge-active' : 'badge-inactive'}">${p.activo ? 'Activo' : 'Pausado'}</span>
      </td>
      <td class="td-actions">
        <div class="action-cell">
          ${p.activo ? `
...
/* =====================================================
   MODAL: NUEVO GASTO (inmediato o programado)
===================================================== */
...
async function saveGasto() {
  const tipo        = document.getElementById('gasto-tipo-flujo').value; // inmediato | programado
...
/* -----------------------------------------------------
   GASTO INMEDIATO
   -> Caja disminuye inmediatamente
   -> Genera movimiento financiero vía CajaAPI
----------------------------------------------------- */
async function registrarGastoInmediato({ categoria, concepto, monto, fecha, metodoId, metodoNombre, observaciones, empleado }) {
  ...
  // 2. Registrar movimiento en Caja usando la API pública ya existente
  const mov = await window.CajaAPI.registrarMovimiento({
    auth_user_id:        STATE.userId,
    tipo_flujo:           'EGRESO',
  ...
  if (!mov.ok) throw new Error(mov.error || 'No se pudo registrar el movimiento en Caja');

  // 3. Vincular el movimiento financiero al gasto (trazabilidad)
  const movId = await getUltimoMovimientoId();
  if (movId) await sbClient.from('gastos').update({ movimiento_financiero_id: movId }).eq('id', gastoRow.id);
}

// Helper: obtiene el id del último movimiento_financiero insertado por este usuario
// (CajaAPI.registrarMovimiento no retorna el id directamente, solo el saldo resultante)
...
/* -----------------------------------------------------
   GASTO PROGRAMADO
   CASO 1: pagarYa = true  -> Caja disminuye + historial + siguiente vencimiento
   CASO 2: pagarYa = false -> Solo se crea la programación, Caja NO se toca
----------------------------------------------------- */
async function crearGastoProgramado({ categoria, nombre, monto, fecha, frecuencia, metodoId, metodoNombre, observaciones, empleado, pagarYa }) {
  // La fecha_proxima inicial es la fecha indicada por el usuario.
  // Si se paga ya, después del pago se recalcula al siguiente ciclo.
  ...
  if (!pagarYa) {
    // CASO 2: Solo programar. No tocar Caja, no crear `gastos` aún.
    return;
  }

  // CASO 1: Registrar primer pago inmediatamente
  await ejecutarPagoProgramado(progRow, { fecha, metodoId, metodoNombre, observaciones });
}

/* -----------------------------------------------------
   Ejecuta un pago (ya sea el primero o uno posterior)
   sobre un gasto programado:
   -> Inserta en `gastos` (tipo='programado')
   -> Inserta en `historial_gastos`
   -> Registra movimiento en Caja vía CajaAPI
   -> Actualiza fecha_proxima del programado según frecuencia
----------------------------------------------------- */
async function ejecutarPagoProgramado(programado, { fecha, metodoId, metodoNombre, observaciones }) {
  ...
  // 3. Insertar en historial_gastos
  ...
  // 4. Calcular y actualizar siguiente vencimiento
...
/* =====================================================
   MODAL: REGISTRAR PAGO (de un programado existente)
===================================================== */
...
async function togglePausarProgramado(id, activar) {
  try {
    await sbClient.from('gastos_programados').update({ activo: activar }).eq('id', id).eq('auth_user_id', STATE.userId);
...
/* =====================================================
   CANCELAR GASTO (nunca se elimina, se marca cancelado)
===================================================== */
    ...
    try {
      ...
      if (hist && hist.length) {
        historialHtml = hist.map(h => `
          <div class="detalle-row"><span>${fmtDate(h.fecha_pago)}</span><strong>${fmt(h.monto)}</strong></div>
        `).join('');
      }
    } catch(e) { /* silencioso */ }
  }
...
function populateCategoriaSelects() {
  ...
  const freqSelects = [document.getElementById('gasto-frecuencia'), document.getElementById('edit-prog-frecuencia')];
...
async function initGastos() {
  ...
  try {
    ...
    // 5. Métodos de pago (de Caja, solo lectura)
    await loadMetodosPago();

    // 6. Datos principales
    await refrescarTodo();

    // Si llega ?action=new desde el dashboard, abrir modal directo
